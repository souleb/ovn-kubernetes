...
// HasDPUNodeReady returns true if the 'ovnDPUNodeReady' node annotation is set.
func HasDPUNodeReady(node *kapi.Node) bool {
	_, ok := node.Annotations[ovnDPUNodeReady]
	return ok
}

// UnsetDPUNodeReady removes the 'ovnDPUNodeReady' node annotation.
func UnsetDPUNodeReady(nodeAnnotator kube.Annotator) {
	nodeAnnotator.Delete(ovnDPUNodeReady)
}
